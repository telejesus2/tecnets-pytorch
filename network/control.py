...
class ControlNet(nn.Module):
    def __init__(self, img_shape, state_dim, action_dim,
        ...
        Input to convolution layers: concatenation of an image (of shape img_shape) and a vector (of dim emb_dim) 
        Input to linear layers: concatenation of the flattened convolution output and a vector (of dim state_dim)
        Output: a vector (of dim action_dim)
        ...
        channels = [img_shape[2] + emb_dim] + filters 
        ...
        fc_nodes = [channels[-1] * conv_h[-1] * conv_w[-1] + state_dim] + fc_layers     
        ...
        self.out = nn.Linear(fc_nodes[-1], action_dim)
    ...
    def forward(self, vision, sentence, state):
        """
        :param sentence: shape (N, emb_dim)
        :param vision: shape (N, h, w, c)
        :param state: shape (N, state_dim)
        """ 
        sentence = sentence.view(-1, sentence.shape[1], 1, 1).expand(-1, sentence.shape[1], vision.shape[1], vision.shape[2]) # (N, emb_dim, h, w)
        vision = vision.permute(0,3,1,2) # (N, c, h, w)
        x = torch.cat((vision, sentence), 1) # (N, c + emb_dim, h, w)
        ...
        x = torch.flatten(x, 1)
        x = torch.cat((x, state), 1) # (N, channels[-1] * conv_h[-1] * conv_w[-1] + state_dim)
